...
export default function Profile() {
    ...
    const getLunchGroup = async (userId) => {
        try {
            const response = await fetch(`/api/getLunchGroup?userId=${encodeURIComponent(userId)}`, {
                method: "GET",
                headers: {
            ...
            console.error("Error getting user meal group", error);
            return null;
        }
    };
    ...
    useEffect(() => {
        if (userId && isLoaded) {
            const fetchLunchGroup = async () => {
                try{
                    const result = await getLunchGroup(userId);
                    console.log(result.data);
                    setLunchGroup(result.data.lunchGroup);
                } catch (error){
                    console.error("Error getting lunch group", error);
                }
            }
            fetchLunchGroup();
        }
